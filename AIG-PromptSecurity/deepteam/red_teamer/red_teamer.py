...
class RedTeamer:
    ...
    def red_team(
        ...
        logger.new_plan_step(newPlanStep(stepId="2", title="Jailbreaking"))
        if self.async_mode:
            ...
            with capture_red_teamer_run(
                ...
                logger.new_plan_step(newPlanStep(stepId="3", title="Risk Assessment"))
                for idx, (vulnerability_type, simulated_attacks) in enumerate(vulnerability_type_to_attacks_map.items()):
                    ...
                    logger.status_update(statusUpdate(stepId="3", brief="Risk Assessment", description=f"Measure for {vulnerability_type.value}"))
                    logger.tool_used(toolUsed(stepId="3", tool_id="Metric measure", brief=f"Measure {num_simulated_attacks} simulated attacks", status="todo"))
                    for _idx, simulated_attack in enumerate(simulated_attacks):
                        logger.tool_used(toolUsed(stepId="3", tool_id="Metric measure", brief=f"Measure {_idx + 1} / {num_simulated_attacks} simulated attacks", status="doing"))
                        ...
                        logger.tool_used(toolUsed(stepId="3", tool_id="Metric measure", brief=f"Generating {_idx + 1} / {num_simulated_attacks} output from simulated attacks", status="doing"))
                        try:
                        ...
                        logger.tool_used(toolUsed(stepId="3", tool_id="Metric measure", brief=f"Evaluating {_idx + 1} / {num_simulated_attacks} target LLM output", status="doing"))
                        try:
                        ...
                        pbar.update(1)
                    logger.tool_used(toolUsed(stepId="3", tool_id="Metric measure", tool_name="Metric measure", brief=f"Measure {num_simulated_attacks} simulated attacks done", status="done"))
                pbar.close()
                ...
                for vuln_type in vulnerability.get_types():
                    ...
                    if metric:
                        ...
                        metrics_map[vuln_type] = lambda: HarmMetric(
                            model=self.evaluation_model,
                            harm_category=f"individuals or society",
                            async_mode=self.async_mode,
                        )
