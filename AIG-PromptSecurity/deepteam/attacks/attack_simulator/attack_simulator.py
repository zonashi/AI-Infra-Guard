...
class AttackSimulator:
    ...
    def simulate(
        ...
        logger.status_update(statusUpdate(stepId="2", brief="Jailbreaking", description=f"Generating attacks"))
        
        logger.tool_used(toolUsed(stepId="2", tool_id="Simulate baseline attacks", brief=f"Simulating {num_vulnerabilities} attacks", status="todo"))
        for idx, vulnerability in enumerate(pbar):
            logger.tool_used(toolUsed(stepId="2", tool_id="Simulate baseline attacks", tool_name="Simulate baseline attacks", brief=f"Simulating {idx} / {num_vulnerabilities} attacks", status="doing"))
        ...
        logger.tool_used(toolUsed(stepId="2", tool_id="Simulate baseline attacks", tool_name="Simulate baseline attacks", brief=f"Simulating {len(vulnerabilities)} attacks done", status="done"))
        ...
        logger.status_update(statusUpdate(stepId="2", brief="Jailbreaking", description=f"Enhance {num_baseline_attacks} attacks"))
        for index, baseline_attack in enumerate(pbar):
            

            if choice == "serial":
                logger.tool_used(toolUsed(stepId="2", tool_id="Enhance attacks", brief=f"Enhance {index + 1} / {num_baseline_attacks} attacks by {choice}", status="todo"))
                logger.tool_used(toolUsed(stepId="2", tool_id="Enhance attacks", brief=f"Enhance {index + 1} / {num_baseline_attacks} attacks with {' '.join([attack.get_name() for attack in attacks])}", status="doing"))
                ...
                logger.tool_used(toolUsed(stepId="2", tool_id="Enhance attacks", tool_name="Enhance attacks", brief=f"Enhance {index + 1} / {num_baseline_attacks} attacks by {choice} done", status="done"))

            elif choice == "parallel":
                for idx, attack in enumerate(attacks):
                    logger.tool_used(toolUsed(stepId="2", tool_id=f"Enhance attacks {idx}", brief=f"Enhance {index + 1} / {num_baseline_attacks} attacks by {choice}", status="todo"))
                    logger.tool_used(toolUsed(stepId="2", tool_id=f"Enhance attacks {idx}", brief=f"Enhance {index + 1} / {num_baseline_attacks} attacks with {attack.get_name()}", status="doing"))
                    ...
                    enhanced_attacks.append(enhanced_attack)
                    logger.tool_used(toolUsed(stepId="2", tool_id=f"Enhance attacks {idx}", tool_name="Enhance attacks", brief=f"Enhance {index + 1} / {num_baseline_attacks} attacks with {attack.get_name()} done", status="done"))
            else:
                # 随机选择攻击：保持原有逻辑
                logger.tool_used(toolUsed(stepId="2", tool_id=f"Enhance attacks", brief=f"Enhance {index + 1} / {num_baseline_attacks} attacks by {choice}", status="todo"))
                attack_weights = [attack.weight for attack in attacks]
                sampled_attack = random.choices(
                    attacks, weights=attack_weights, k=1
                )[0]
                logger.tool_used(toolUsed(stepId="2", tool_id=f"Enhance attacks", brief=f"Enhance {index + 1} / {num_baseline_attacks} attacks with {sampled_attack.get_name()}", status="doing"))
                enhanced_attack = self.enhance_attack(
                ...
                enhanced_attacks.append(enhanced_attack)
                logger.tool_used(toolUsed(stepId="2", tool_id="Enhance attacks", tool_name="Enhance attacks", brief=f"Enhance {index + 1} / {num_baseline_attacks} attacks with {sampled_attack.get_name()} done", status="done"))
    ...
    async def a_simulate(
        ...
        async def throttled_simulate_baseline_attack(vulnerability):
            logger.tool_used(toolUsed(stepId="2", tool_id="Simulate baseline attacks", tool_name="Simulate baseline attacks", brief=f"Simulating async attacks", status="doing"))
        ...
        pbar.close()
        logger.tool_used(toolUsed(stepId="2", tool_id="Simulate baseline attacks", tool_name="Simulate baseline attacks", brief=f"Simulating {num_vulnerabilities} attacks done", status="done"))
        ...
        logger.status_update(statusUpdate(stepId="2", brief="Jailbreaking", description="Enhance attacks"))
        logger.tool_used(toolUsed(stepId="2", tool_id="Enhance attacks", brief=f"Enhance {num_baseline_attacks} attacks", status="todo"))
    ...
    async def a_simulate_baseline_attacks(
        ...
        for vulnerability_type in vulnerability.get_types():
            try:
                if isinstance(vulnerability, CustomPrompt):
                    local_attacks = vulnerability.custom_prompt
                else:
    ...
    def enhance_attack_serial(
        ...
        try:
            for i, attack in enumerate(attacks):
                ...
                logger.debug(f"Input length changed from {len(attack_input) if i == 0 else len(attacks[i-1].enhance(attack_input))} to {len(current_input)}")
