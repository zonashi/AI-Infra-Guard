...
def parse_vulnerability(arg: str, plugin_manager: PluginManager) -> List[Any]:
    """解析漏洞参数"""
    if ":" in arg:
        class_name, param_str = arg.split(":", 1)
        ...
        if class_name == "Custom":
            ...
            csv_file = kwargs.get('csv_file', 'jb-top100.csv')
        ...
        if class_name == "Custom":
            ...
            from deepteam.vulnerabilities import MultiDatasetVulnerability
            return [MultiDatasetVulnerability()]
        else:
            # 如果不是自定义插件，使用内置映射
            class_path = SCENARIO_CLASS_MAP.get(class_name)
            if not class_path:
